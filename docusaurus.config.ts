...
function generateSocialIconsHtml(socialLinks?: typeof siteConfig.footer.socialLinks): string {
  ...
  const linksHtml = socialLinks.map(link => 
    `<a href="${link.url}" aria-label="${link.platform}" target="_blank" rel="noopener noreferrer">${icons[link.platform] || ''}</a>`
  ).join('\n                  ');
...
// Build footer links with social icons
const footerLinks = [
  ...siteConfig.footer.links,
  ...(siteConfig.footer.socialLinks && siteConfig.footer.socialLinks.length > 0 ? [{
    title: 'Connect',
    items: [{
      html: generateSocialIconsHtml(siteConfig.footer.socialLinks),
    }],
  }] : []),
];
    ...
    [
      'classic',
      {
        docs: {
          ...
          routeBasePath: 'docs', // Docs at /docs path
        },
        blog: {
  ...
  themeConfig: {
    ...
    navbar: {
      ...
      logo: {
        ...
        srcDark: siteConfig.logo.srcDark,
        href: '/',
      },
      items: siteConfig.navbar.items as any[],
    },
    footer: {
      style: siteConfig.footer.style,
      links: footerLinks,
    ...
    prism: {
      ...
      additionalLanguages: ['bash', 'json', 'yaml', 'python', 'java', 'typescript', 'sql'],
    },
    liveCodeBlock: {
